...
- **Author styles as `.sass` (indented syntax), never `.css`** — every component's `style.sass`, this boilerplate's own `src/style.sass`/`src/fonts.sass`/`src/styles/_mixins.sass`/`_effects.sass` are the reference. This is about our own hand-authored stylesheets specifically — it does *not* apply to vendored or tool-generated CSS we don't hand-edit (a self-hosted Google Fonts `@font-face` file, a Flaticon/IcoMoon icon-font export, a vendored library like Bootstrap) — those stay as `.css` since they'd be regenerated/replaced wholesale, not edited by hand. Note `.sass`'s indented syntax has no braces or semicolons — it is not a drop-in rename of a `.css` file; converting one means actually rewriting it to the indented syntax, not just changing the extension.

## Package identity

- Rename `package.json`'s `"name"` away from `dsplay-template-react-boilerplate` immediately when starting a new template from this boilerplate — it must identify the template itself, not the boilerplate it was cloned from. Convention: `dsplay-` + this repo's own GitHub name (e.g. the [`template-alerts`](https://github.com/dsplay/template-alerts) repo → `dsplay-template-alerts`).
...
- [`@dsplay/react-template-utils`](https://github.com/dsplay/react-template-utils) (which wraps [`@dsplay/template-utils`](https://github.com/dsplay/template-utils)) exposes `useTemplateVal`/`useTemplateBoolVal`/`useTemplateIntVal`/`useTemplateFloatVal` (read a `dsplay_template` variable as string/boolean/integer/float, with an optional default), plus `useTemplate()`/`useMedia()`/`useConfig()` for the raw objects.
- **Always read template data through these hooks, called inside the function component that uses the value — never call `@dsplay/template-utils`'s vanilla `tval`/`tbval`/`tival`/`tfval`/`config` directly, and never read them at module scope as a one-time constant.** `@dsplay/template-utils` should not appear as a direct dependency in a React template's `package.json` at all (it's still pulled in transitively via `@dsplay/react-template-utils`). Module-scope reads happen to work because DSPLAY injects its data before any module executes, but they break React's reactivity model (a value can't ever update after first render) and were a bug source when [`template-horizontal-info-bar`](https://github.com/dsplay/template-horizontal-info-bar) had them — treat any vanilla `@dsplay/template-utils` import in a React template's `src/` as something to migrate, not a style choice.
...
- `Loader`'s `tasks` prop (`src/components/app/index.jsx`) accepts promises to await before the intro placeholder is dismissed - e.g. fetching data the template needs before it can render. Their settled values are exposed via `LoaderContext` as `tasksResults`, in order. A rejected task does **not** block the others or leave the intro stuck forever (as of `@dsplay/react-template-utils@5.2.1` - older versions used `Promise.all` and would hang here): it settles as `undefined` in `tasksResults` at that index, with the rejection reason exposed in parallel via `tasksErrors`. `src/components/app/index.jsx` and `src/components/main/index.jsx` include a small working example (a task that deliberately fails, plus reading `tasksErrors` for it in `Main`) - see [`template-flight-information`](https://github.com/dsplay/template-flight-information)'s AGENTS.md/`use-flights-info.js` for a complete production example built on this (a translated error message shown only when there's truly no data to fall back on).
- New `dsplay_template` keys should be `snake_case` (see "README structure" above) — the CMS Manager derives each variable's on-screen label from its key name.
...
This same section (and the config it describes) should be carried into every new template cloned from this boilerplate — a full fleet audit in 2026 found 7 real templates had silently lost the `Chrome >= 45`/`Android >= 4.4` browserslist entries over time despite `@vitejs/plugin-legacy` still being nominally configured, so treat this as a checked invariant, not incidental boilerplate content.
